...
class LogViewServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        if (method_exists($this, 'loadViewsFrom')) {
            $this->loadViewsFrom(__DIR__.'/views', 'laravel-log-view');
        }

        if (method_exists($this, 'publishes')) {
            $this->publishes([
                __DIR__.'/views' => base_path('/resources/views/vendor/laravel-log-view'),
            ], 'views');

            $this->publishes([
                __DIR__.'/config/logview.php' => $this->config_path('logview.php'),
            ]);
